...
func TestApphostPackagePreparationIsRepeatableAndPrivate(t *testing.T) {
	if _, err := exec.LookPath("rsync"); err != nil {
		t.Skip("rsync is unavailable")
	}
	output := filepath.Join(t.TempDir(), "package")
	for attempt := 0; attempt < 2; attempt++ {
		command := exec.Command("scripts/prepare-apphost-package.sh", "--output", output)
	...
	if _, err := os.Stat(filepath.Join(output, ".zak-radio-apphost-package")); err != nil {
	...
	command := exec.Command("scripts/prepare-apphost-package.sh",
		"--output", external,
	...
	sealedRouting, err := os.ReadFile(
		filepath.Join(external, "internal", "application", "sealed_routing_generated.go"))
	if err != nil {
		t.Fatal(err)
	}
	for _, expected := range []string{
		`packagedAllowedHosts = "music.example,radio.example,loopback"`,
		`packagedAllowedOrigins = "https://music.example,https://radio.example,loopback"`,
		`packagedTrustedProxies = "10.0.0.2"`,
		`packagedTrustedIngress = "10.0.0.2"`,
	} {
		if !bytes.Contains(sealedRouting, []byte(expected)) {
			t.Fatalf("external package omitted %q", expected)
		}
	}
	manifest, err := os.ReadFile(filepath.Join(external, "PACKAGE.SHA256SUMS"))
	if err != nil {
		t.Fatal(err)
	}
	manifestSum := sha256.Sum256(manifest)
	if fmt.Sprintf("%x", manifestSum) != strings.TrimSpace(string(externalRelease)) {
		t.Fatal("external package release does not bind its build-input manifest")
	}
	if bytes.Contains(dockerfile, []byte("ARG ZAK_RADIO")) {
		t.Fatal("package Dockerfile retains mutable routing or release build arguments")
	}
	for _, exactControl := range []string{
		"! -path ./RELEASE",
		"! -path ./PACKAGE.SHA256SUMS",
		"! -path ./.zak-radio-apphost-package",
	} {
		if !bytes.Contains(dockerfile, []byte(exactControl)) {
			t.Fatalf("package Dockerfile does not exclude only root control %q",
				exactControl)
		}
	}
	...
	replace := exec.Command("scripts/prepare-apphost-package.sh",
		"--output", output,
	...
	if result, err := replace.CombinedOutput(); err == nil ||
		!strings.Contains(string(result), "immutable release package") {
	...
	partial := exec.Command("scripts/prepare-apphost-package.sh",
	...
	command = exec.Command("scripts/prepare-apphost-package.sh", "--output", staticOutput)
	if result, err := command.CombinedOutput(); err == nil {
...
func writeTestPackageManifest(t *testing.T, root string) {
	...
	err := filepath.WalkDir(root, func(path string, entry os.DirEntry, walkErr error) error {
		...
		case "RELEASE", "PACKAGE.SHA256SUMS", ".zak-radio-apphost-package":
			return nil
		}
	...
	release := sha256.Sum256(manifest.Bytes())
	if err := os.WriteFile(
		filepath.Join(root, "RELEASE"), []byte(fmt.Sprintf("%x\n", release)), 0o640,
	); err != nil {
		t.Fatal(err)
	}
}

func TestPackagedRoutingRejectsRuntimeOverride(t *testing.T) {
	previous := []any{
		packagedRoutingSealed, packagedAllowedHosts, packagedAllowedOrigins,
		packagedTrustedProxies, packagedTrustedIngress,
	}
	t.Cleanup(func() {
		packagedRoutingSealed = previous[0].(bool)
		packagedAllowedHosts = previous[1].(string)
		packagedAllowedOrigins = previous[2].(string)
		packagedTrustedProxies = previous[3].(string)
		packagedTrustedIngress = previous[4].(string)
	})
	packagedRoutingSealed = true
	packagedAllowedHosts = "radio.example,loopback"
	packagedAllowedOrigins = "https://radio.example,loopback"
	packagedTrustedProxies = "192.0.2.1"
	packagedTrustedIngress = "192.0.2.1"
	t.Setenv("ZAK_RADIO_ALLOWED_HOSTS", "attacker.example")
	if _, err := defaultConfig(); err == nil ||
		!strings.Contains(err.Error(), "immutable packaged routing") {
		t.Fatalf("sealed routing override error=%v", err)
	}
	if err := validatePackagedRouting(Config{
		AllowedHosts: "attacker.example", AllowedOrigins: packagedAllowedOrigins,
		TrustedProxies: packagedTrustedProxies, TrustedIngress: packagedTrustedIngress,
	}); err == nil {
		t.Fatal("packaged command-line routing override was accepted")
	}
}
...
func TestApphostPackageRejectsStaticSymlink(t *testing.T) {
