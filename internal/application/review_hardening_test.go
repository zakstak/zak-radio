...
func TestRuntimeVolumeLockExcludesConcurrentUsers(t *testing.T) {
	cfg := newTestConfig(t)
	first, err := acquireRuntimeVolumeLock(cfg.MetadataRoot)
	if err != nil {
		t.Fatal(err)
	}
	if second, err := acquireRuntimeVolumeLock(cfg.MetadataRoot); err == nil {
		second.Close()
		first.Close()
		t.Fatal("second runtime acquired retained-volume lifecycle lock")
	}
...
func TestContendedHealthProbeFailsClosed(t *testing.T) {
	app := newTestApp(t)
	app.healthProbes <- struct{}{}
	defer func() { <-app.healthProbes }()
	req := httptest.NewRequest(http.MethodGet, "/health", nil)
	res := httptest.NewRecorder()
	app.health(res, req)
	if res.Code != http.StatusServiceUnavailable {
		t.Fatalf("contended health status=%d body=%s", res.Code, res.Body.String())
	}
	var payload struct {
	...
	for _, check := range []string{"database_runtime", "journal", "writable"} {
		if payload.Checks[check] {
			t.Fatalf("contended health reused a passing %s result: %#v", check, payload.Checks)
		}
	}
}
