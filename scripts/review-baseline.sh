...
bash -n \
  ...
  scripts/prepare-apphost-package.sh \
...
echo "==> root container input manifest"
! find ./static -mindepth 1 \( -type l -o \( ! -type f ! -type d \) \) \
  -print -quit | grep -q .
[[ "$(cat RELEASE)" == "$(sha256sum PACKAGE.SHA256SUMS | cut -d' ' -f1)" ]]
sha256sum --check PACKAGE.SHA256SUMS
{
  printf '%s\0' ./.dockerignore ./Dockerfile ./apphost.vnext.toml ./go.mod ./go.sum ./package.json ./package-lock.json
  find ./cmd -type f -print0
  find ./internal -type f -print0
  find ./vendor -type f -print0
  find ./static -type f -print0
} | LC_ALL=C sort -zu | xargs -0 sha256sum >"$scratch_dir/root-package-manifest"
cmp PACKAGE.SHA256SUMS "$scratch_dir/root-package-manifest"
...
ops_image="$(docker build --quiet -f scripts/Dockerfile.review-ops scripts)"
docker run --rm --user 0:0 \
...
echo "==> policy-safe Apphost package and clean container build"
  package_dir="$scratch_dir/apphost-package"
  manifest="$(scripts/prepare-apphost-package.sh --output "$package_dir")"
  [[ "$manifest" == "$package_dir/apphost.vnext.toml" ]]
  release_id="$(cat "$package_dir/RELEASE")"
  [[ "$release_id" =~ ^[0-9a-f]{64}$ ]]
  [[ "$release_id" == "$(sha256sum "$package_dir/PACKAGE.SHA256SUMS" | cut -d' ' -f1)" ]]
  (cd "$package_dir" && sha256sum --check PACKAGE.SHA256SUMS)
  ! grep -q 'ARG ZAK_RADIO_RELEASE' "$package_dir/Dockerfile"
  ...
  [[ "$(docker image inspect --format '{{.Config.User}}' "$image_id")" == "65532:65532" ]] || {
    echo "container runtime user is not 65532:65532" >&2
    exit 1
  }
  inspect_static_mode() {
  ...
  inspect_static_mode "$image_id" "apphost-package"
  root_image_id="$(docker build --quiet "$repo_root")"
  [[ "$(docker image inspect --format '{{.Config.User}}' "$root_image_id")" == "65532:65532" ]]
  inspect_static_mode "$root_image_id" "root"
