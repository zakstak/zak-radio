...
def fetch_pages(base: str, path: str, key: str):
    ...
    while True:
        ...
        if (
            ...
            raise ValueError(
                f"{key} pagination cursor did not advance: {next_offset!r}"
            )
        offset = next_offset
...
def check_range(url: str) -> tuple[int, str | None, int]:
    ...
    try:
        ...
        return (
            err.code,
            err.headers.get("Content-Range"),
            len(err.read(MAX_ERROR_BYTES + 1)),
        )
...
def check_origin_policy(base: str) -> tuple[int, str]:
    ...
    try:
        with open_request(request) as response:
            return response.status, response.read(MAX_ERROR_BYTES + 1).decode(
                "utf-8", "replace"
            )
    except urllib.error.HTTPError as err:
        try:
            return err.code, err.read(MAX_ERROR_BYTES + 1).decode("utf-8", "replace")
        finally:
            err.close()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify Zak Radio + Reader runtime endpoints"
    )
    ...
    if (
        args.expected_tracks <= 0
        or args.expected_reader_items < 0
        or args.deadline_seconds < 1
        or args.deadline_seconds > 3600
    ):
        parser.error(
            "expected tracks must be positive and expected Reader items non-negative"
        )
    ...
    if status != 200 or len(items) != args.expected_reader_items:
        print(
            f"reader items failed: status={status} count={len(items)}", file=sys.stderr
        )
        return 1
    ...
    for track in tracks:
        ...
        if (
            head_status != 200
            or expected_bytes <= 0
            or content_length != expected_bytes
    ...
    for item in items:
        ...
        if source_status != 200:
            print(
                f"reader source failed: item={item_id} status={source_status}",
                file=sys.stderr,
            )
            return 1
        ...
        if status != 200:
            print(
                f"reader segments failed: item={item_id} status={status}",
                file=sys.stderr,
            )
            return 1
        ...
        if indices != list(range(len(indices))):
            print(
                f"reader segment indices are not contiguous: item={item_id} indices={indices}",
                file=sys.stderr,
            )
            return 1
        ...
        for segment in ready:
            ...
            media_status, actual_bytes, actual_digest = check_digest(
                url, expected_bytes
            )
            if (
                media_status != 200
    ...
    for url, expected_size in dict(range_urls).items():
        ...
        if (media_status, media_len) != (206, 100) or not valid_content_range(
            media_range, expected_size
        ):
            print(
