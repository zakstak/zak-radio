...
for control in RELEASE PACKAGE.SHA256SUMS .zak-radio-apphost-package; do
...
[[ "$release" =~ ^[0-9a-f]{64}$ &&
   "$release" == "$(sha256sum "$package_root/PACKAGE.SHA256SUMS" | cut -d' ' -f1)" ]] || {
...
cmp -s \
  <(cd "$package_root" && find . -type f ! -path './RELEASE' \
    ! -path './PACKAGE.SHA256SUMS' ! -path './.zak-radio-apphost-package' \
