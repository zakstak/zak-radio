...
class VerifierSafetyTests(unittest.TestCase):
    ...
    def test_content_range_requires_requested_offsets_and_total(self):
        ...
        for value in (None, '', 'bytes 100-199/200', 'bytes 0-98/200',
                      'bytes 0-99/201', 'garbage'):
    ...
    def test_digest_rejects_bytes_beyond_declared_size(self):
        ...
        try:
            with self.assertRaises(ValueError):
                VERIFY.check_digest(
                    f'http://127.0.0.1:{server.server_port}/media', 4)
        finally:
            server.shutdown()
            thread.join()
            server.server_close()
    ...
    def test_origin_probe_is_non_mutating_and_uses_public_origin(self):
        ...
        class OriginHandler(BaseHTTPRequestHandler):
            def do_POST(self):
                ...
                self.send_response(400 if observed['origin'] ==
