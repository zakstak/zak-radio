...
const storedPlaybackRate = Number(window.ZakStorage.get('readerPlaybackRate', '1.15'));
const state = {
  items: [],
  filtered: [],
  item: null,
  ...
  totalDuration: 0,
  position: 0,
  playbackRate: supportedPlaybackRates.includes(storedPlaybackRate) ? storedPlaybackRate : 1.15,
  lastSavedAt: 0,
  saveTimer: 0,
  loadGeneration: 0,
  loadAbort: null,
...
function sizeLabel(bytes) {
  ...
  return Number.isFinite(value) && value > 0 ? window.ZakFormatBytes(value) : 'No audio';
}
...
function readerWriterID() {
  ...
  state.writerID = Array.from(bytes, (value) => value.toString(16).padStart(2, '0')).join('');
...
async function paged(path, key, options = {}) {
  ...
  for (;;) {
    ...
    if (++pages > 1000) throw new Error('Pagination exceeded its page budget');
    const separator = path.includes('?') ? '&' : '?';
    const pagination = cursor
      ? `cursor=${encodeURIComponent(cursor)}`
      : `offset=${offset}`;
    const payload = await api(`${path}${separator}${pagination}`, requestOptions);
    for (const value of payload[key] || []) {
      const identity = value && typeof value === 'object' && value.id !== undefined
        ? `id:${value.id}`
        : value && typeof value === 'object' && value.segment_index !== undefined
    ...
    if (payload.next_cursor !== undefined && payload.next_cursor !== null) {
      if (typeof payload.next_cursor !== 'string' || !payload.next_cursor ||
          seenCursors.has(payload.next_cursor)) throw new Error('Pagination cursor did not advance');
      cursor = payload.next_cursor;
      continue;
    }
    if (payload.next_offset === undefined || payload.next_offset === null) return result;
    const next = Number(payload.next_offset);
    if (!Number.isInteger(next) || next <= offset) throw new Error('Pagination offset did not advance');
    offset = next;
  }
}
...
function renderList() {
  ...
  state.filtered = state.items.filter((item) => !q || `${item.title} ${item.source_url || ''} ${item.status}`.toLowerCase().includes(q));
  ...
  for (const item of state.filtered) {
    ...
    button.className = isCurrent ? 'reader-list-item is-active' : 'reader-list-item';
    button.dataset.itemId = item.id;
    if (isCurrent) button.setAttribute('aria-current', 'true');
    const title = document.createElement('div');
    title.className = 'reader-list-title';
    title.textContent = item.title;
    const sub = document.createElement('div');
    sub.className = 'reader-list-meta';
    sub.textContent = `${item.status} · ${t(item.total_duration)} · ${new Date(item.uploaded_at * 1000).toLocaleString()}`;
    button.append(title, sub);
    button.onclick = () => loadItem(item.id, { historyMode: 'push', focusDetail: true });
    els.items.append(button);
  }
}
...
function renderLibrary() {
  ...
  const ownsReader = window.ZakAudio.is('reader') && Boolean(state.activePlayback);
  ...
  grid.className = 'reader-library-grid';
  for (const item of state.filtered) {
    ...
    meta.textContent = `${item.status} · ${t(item.total_duration)} · ${item.segment_count} segments · ${sizeLabel(item.audio_bytes)}`;
    const url = document.createElement('div');
    url.className = 'reader-card-url';
    url.textContent = item.source_url || item.source_type;
    card.append(title, meta, url);
    card.onclick = () => loadItem(item.id, { historyMode: 'push', focusDetail: true });
    grid.append(card);
  }
  if (!state.filtered.length) {
    ...
    if (state.items.length) {
      heading.textContent = 'Nothing matches this search.';
      empty.append(heading, 'Clear the search or try a different phrase.');
    } else {
      heading.textContent = 'The reading room is quiet.';
      empty.append(heading, 'Saved documents will appear here when they are ready.');
    }
    grid.append(empty);
  }
  els.text.append(grid);
  renderList();
  renderPlaybackState();
}
...
function absoluteOffset() {
  ...
  for (let i = 0; i < state.index; i++) {
    if (isPlayable(state.segments[i])) x += segmentDuration(state.segments[i]);
  }
  const ownsSelected = window.ZakAudio.is('reader') &&
    state.activePlayback?.itemID === state.item?.id &&
    Number(state.activePlayback?.segmentIndex) === Number(state.segments[state.index]?.segment_index);
  const position = ownsSelected && !els.audio.ended ? els.audio.currentTime || 0 : state.position;
  return x + position;
}
...
function renderProgress() {
  ...
  const percent = state.totalDuration > 0 ? Math.max(0, Math.min(100, (pos / state.totalDuration) * 100)) : 0;
...
function updateReaderPositionState() {
  if (!('mediaSession' in navigator) || !window.ZakAudio.is('reader') ||
      !state.item || state.totalDuration <= 0) return;
  const position = Math.max(0, Math.min(absoluteOffset(), state.totalDuration));
  try {
...
function applyPlaybackRate() {
  if (!supportedPlaybackRates.includes(state.playbackRate)) state.playbackRate = 1.15;
  if (els.speed) els.speed.value = String(state.playbackRate);
  if (window.ZakAudio.is('reader')) els.audio.playbackRate = state.playbackRate;
  updateReaderPositionState();
}
...
function loadAudio(position = 0, play = false) {
  ...
  if (play && position >= segmentDuration(seg) && nextPlayableIndex(state.index) < 0) {
    state.index = state.segments.findIndex(isPlayable);
    position = 0;
  ...
  const changed = window.ZakAudio.claim('reader', src, state.item.title || 'Reader audio');
  ...
  if (!changed) els.audio.currentTime = position;
    ...
    state.metadataHandler = () => {
      state.metadataHandler = null;
      if (!window.ZakAudio.is('reader') || window.ZakAudio.generation !== generation ||
          !els.audio.currentSrc.endsWith(src)) return;
    els.audio.currentTime = position;
      if (play) window.ZakAudio.play();
    };
    els.audio.addEventListener('loadedmetadata', state.metadataHandler, { once: true });
  }
  applyPlaybackRate();
  highlight();
...
function refreshTotalDuration() {
  state.totalDuration = state.segments.filter(isPlayable)
...
function imagesForSegment(seg) {
  ...
  return state.images.filter((img) => (img.figure || '').trim().toLowerCase() === key);
}
...
function appendImage(img) {
  ...
  image.addEventListener('error', () => {
    ...
    retry.onclick = () => loadItem(state.item.id, { historyMode: 'none', background: true });
...
function renderText() {
  ...
  if (state.resumeError) {
    ...
    warning.textContent = 'Saved listening progress is temporarily unavailable. Playback is paused to protect your last position. ';
    const retry = document.createElement('button');
    retry.type = 'button';
    retry.className = 'button';
  ...
  if (!state.segments.length) {
    ...
    empty.className = 'reader-empty';
    const heading = document.createElement('strong');
    heading.textContent = state.item.status === 'ready' ? 'No readable segments were produced.' : 'Audio is still being prepared.';
    empty.append(heading, 'The preserved Source remains available while Reader processing completes.');
  ...
  for (const seg of state.segments) {
    ...
    control.disabled = seg.status !== 'ready' ||
      state.resumeReady[state.item.id] !== true;
    control.setAttribute('aria-label', `${seg.kind === 'heading' ? 'Heading' : 'Read'}: ${seg.text}`);
    control.onclick = () => {
...
function renderTextPreservingFocus() {
  const focusedSegment = document.activeElement?.closest?.('[data-segment]')?.dataset.index;
...
function updateReaderURL(id, mode) {
  ...
  if (`${url.pathname}${url.search}` !== `${location.pathname}${location.search}`) {
    history[method](method === 'replaceState' ? history.state || {} : { scroll: [0, 0] }, '', url);
  }
}
...
function showReaderError(error, id) {
  ...
  els.details.textContent = `${id} could not be loaded. ${error.message || ''}`.trim();
  ...
  retry.onclick = () => loadItem(id, { historyMode: 'replace', focusDetail: true });
  message.append(retry);
  els.text.replaceChildren(message);
  els.title.focus({ preventScroll: true });
}

async function loadItem(id, { historyMode = 'none', focusDetail = false, background = false } = {}) {
  ...
  if (!background) {
    ...
    [els.back, els.playPause, els.forward, els.progress].forEach((control) => {
      control.disabled = true;
    });
    renderProgress();
  }
  try {
    const itemPromise = api(`/api/reader/items/${encoded}`, { signal: controller.signal });
    ...
    const segments = await paged(`/api/reader/items/${encoded}/segments`, 'segments', {
      signal: controller.signal,
      onPage: (partial, payload) => {
        if (background || generation !== state.loadGeneration || controller.signal.aborted ||
            !window.location.pathname.startsWith('/reader')) return;
        state.item = item;
        state.segmentsLoading = payload.next_offset != null;
        state.segments = partial;
        refreshTotalDuration();
        els.title.textContent = item.title;
        setReaderText(els.meta, `${item.status} · ${item.voice} · ${item.source_type}`);
        els.details.textContent = payload.next_offset == null
    ...
    if (playable) {
      try {
        playback = await api(`/api/reader/playback?item_id=${encoded}`, { signal: controller.signal });
      } catch (error) {
        if (error.name === 'AbortError') throw error;
        playbackError = true;
      }
    ...
    if (localPlayback) {
      ...
      if (localPlayback.pending || serverRevision < Number(localPlayback.revision || 0)) {
        playback = {
          ...(playback || {}),
          segment_index: localPlayback.segment_index,
          position: localPlayback.position,
          playing: localPlayback.playing,
          revision: Math.max(serverRevision, Number(localPlayback.revision || 0)),
        };
      } else if (serverRevision >= Number(localPlayback.revision || 0)) {
        delete state.localPlayback[item.id];
      }
    }
    if (generation !== state.loadGeneration || controller.signal.aborted ||
        !window.location.pathname.startsWith('/reader')) return false;
    if (background && (state.item?.id !== id ||
        new URLSearchParams(location.search).get('item') !== id)) return false;
    state.item = item;
    state.showingLibrary = false;
    state.attemptedID = '';
    ...
    if (playbackError && window.ZakAudio.is('reader') &&
        state.activePlayback?.itemID === item.id) {
      stopActivePlayback();
    }
    if (playback) state.playbackRevisions[item.id] = Number(playback.revision || 0);
    refreshTotalDuration();
    const activeIndex = window.ZakAudio.is('reader') && state.activePlayback?.itemID === item.id
      ? segments.findIndex((segment) =>
          Number(segment.segment_index) === Number(state.activePlayback.segmentIndex) && isPlayable(segment))
      : -1;
    const restored = activeIndex >= 0 ? activeIndex : playback
      ? segments.findIndex((segment) =>
          Number(segment.segment_index) === Number(playback.segment_index) && isPlayable(segment))
      : -1;
    state.index = restored >= 0 ? restored : segments.findIndex(isPlayable);
    state.position = activeIndex >= 0
      ? Number(els.audio.currentTime || 0)
      : restored >= 0 ? Number(playback?.position || 0) : 0;
    els.title.textContent = item.title;
    setReaderText(els.meta, `${item.status} · ${item.voice} · ${item.source_type}`);
    ...
    renderText();
    renderList();
    state.needsRefresh = !playable ||
      segments.some((segment) => ['pending', 'processing'].includes(segment.status));
    ...
    if (background) {
      if (focusedSegment !== undefined && focusedSegment !== null) {
        document.querySelector(`[data-index="${CSS.escape(String(focusedSegment))}"] button`)?.focus({ preventScroll: true });
      } else if (focusedItem) {
        document.querySelector(`[data-item-id="${CSS.escape(focusedItem)}"]`)?.focus({ preventScroll: true });
      } else if (focusedAction) {
        (document.querySelector(`[data-reader-action="${CSS.escape(focusedAction)}"]`) ||
          els.title).focus({ preventScroll: true });
      }
      if (readerScrollGeneration === priorScrollGeneration) {
    ...
    if (focusDetail) {
      document.querySelector('.reader-main').scrollIntoView({ block: 'start' });
      els.title.focus({ preventScroll: true });
    }
    void imagePromise.then((imageData) => {
      if (generation !== state.loadGeneration || controller.signal.aborted ||
...
function renderPlaybackState() {
  const playing = Boolean(window.ZakAudio.is('reader') && state.item &&
    state.activePlayback?.itemID === state.item.id && !els.audio.paused);
...
function playbackBody(playing = !els.audio.paused, force = false, explicitPosition = null) {
  const active = state.activePlayback;
  if (!active || (!force && (!window.ZakAudio.is('reader') ||
      active.generation !== window.ZakAudio.generation)) ||
      state.resumeReady[active.itemID] === false) return null;
  window.clearTimeout(state.saveTimer);
  state.saveTimer = 0;
  state.lastSavedAt = Date.now();
  const observed = explicitPosition === null ? els.audio.currentTime || 0 : explicitPosition;
...
async function persistPlayback(body, keepalive = false) {
  try {
    ...
    if (local && Number(local.writer_sequence) === Number(body.writer_sequence)) {
    ...
    if (failed && Number(failed.writer_sequence) <= Number(body.writer_sequence)) {
...
function savePlayback(playing = !els.audio.paused, force = false, explicitPosition = null) {
  const body = playbackBody(playing, force, explicitPosition);
  if (!body) return state.saveQueue;
  state.saveQueue = state.saveQueue.catch(() => {}).then(() => persistPlayback(body)).catch(() => {});
...
function queuePlaybackSave() {
  if (!state.activePlayback || !window.ZakAudio.is('reader') || state.saveTimer) return;
  const delay = Math.max(0, 5000 - (Date.now() - state.lastSavedAt));
  state.saveTimer = window.setTimeout(() => savePlayback(!els.audio.paused), delay);
}
...
async function toggleReaderPlayback() {
  if (!state.item || state.resumeReady[state.item.id] !== true) return;
  const ownsSelected = window.ZakAudio.is('reader') && state.activePlayback?.itemID === state.item.id;
...
els.progress.oninput = () => {
  ...
  const percent = state.totalDuration > 0 ? Math.max(0, Math.min(100, (value / state.totalDuration) * 100)) : 0;
...
els.search.oninput = () => {
  ...
  renderList();
  if (!state.item || state.showingLibrary) renderLibrary();
};
...
async function boot(force = false) {
	  ...
	  state.items = await paged('/api/reader/items', 'items', {
	    ...
	    onPage: (items, payload) => {
	      if (generation !== state.bootGeneration ||
	          !location.pathname.startsWith('/reader')) return;
	      state.items = items;
        state.loaded = true;
        ...
        if (!currentRequested) {
          els.count.textContent = payload.next_offset != null
	...
	try {
	  ...
	  if (error?.name !== 'AbortError' && generation === state.bootGeneration) throw error;
	} finally {
...
if (location.pathname.startsWith('/reader')) boot().catch(showReaderBootError);
...
window.addEventListener('zak-route', async (event) => {
    ...
    state.loadGeneration++;
	    ...
	    if (restoreLibraryFocus) {
	      window.queueMicrotask(() => els.title.focus({ preventScroll: true }));
	    }
	    return;
  }
  ...
  } else if (state.item && (state.needsRefresh || !state.segments.some(isPlayable))) {
...
window.ZakReader = {
  selectedId() {
    return state.showingLibrary ? '' : state.attemptedID || state.item?.id || '';
  },
